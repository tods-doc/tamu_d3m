...
from test_primitives.null import NullTransformerPrimitive, NullDataFrameUnsupervisedLearnerPrimitive
...
EXPECTED_PRIMITIVE_DESCRIPTION_JSON = r"""
{
    ...
    "original_python_path": "test_primitives.null.NullTransformerPrimitive",
    "primitive_code": {
        "class_type_arguments": {
            "Inputs": "d3m.container.list.List",
            "Outputs": "d3m.container.list.List",
            "Hyperparams": "test_primitives.null.Hyperparams",
            "Params": "NoneType"
        },
        ...
        "arguments": {
            "hyperparams": {
                "type": "test_primitives.null.Hyperparams",
                "kind": "RUNTIME"
            },
            "random_seed": {
                "type": "int",
...
""".replace('__INTERFACES_VERSION__', d3m.__version__).replace('__GIT_COMMIT__', utils.current_git_commit(TEST_PRIMITIVES_DIR)).replace('__DIGEST__', NullTransformerPrimitive.metadata.query()['digest'])


class TestNullPrimitives(unittest.TestCase):
    def call_primitive(self, primitive, method_name, extra_arguments=None, **kwargs):
        primitive_arguments = primitive.metadata.query()['primitive_code'].get('arguments', {})
        ...
        for argument_name, argument_value in dict(extra_arguments, **kwargs).items():
            if primitive_arguments[argument_name]['kind'] == base.PrimitiveArgumentKind.PIPELINE:
    ...
    def test_null_transformer(self):
        hyperparams_class = NullTransformerPrimitive.metadata.get_hyperparams()

        primitive = NullTransformerPrimitive(hyperparams=hyperparams_class.defaults())

        inputs = container.List([10, 20, 30], generate_metadata=True)
        call_metadata = self.call_primitive(primitive, 'produce', inputs=inputs)

        self.assertSequenceEqual(call_metadata.value, [10, 20, 30])
    ...
    def test_null_transformer_metadata(self):
        ...
        description = json.loads(json.dumps(NullTransformerPrimitive.metadata.to_json_structure()))

        self.maxDiff = None
        self.assertEqual(expected_description, description)

    def test_null_dataframe_unsupervised_learner(self):
        hyperparams_class = NullDataFrameUnsupervisedLearnerPrimitive.metadata.get_hyperparams()

        primitive = NullDataFrameUnsupervisedLearnerPrimitive(hyperparams=hyperparams_class.defaults())

        inputs = container.DataFrame([10, 20, 30], generate_metadata=True)
        call_metadata = self.call_primitive(primitive, 'produce', inputs=inputs)

        self.assertSequenceEqual(call_metadata.value.values.tolist(), [[10], [20], [30]])
        self.assertEqual(call_metadata.has_finished, True)
        self.assertEqual(call_metadata.iterations_done, None)

        self.assertEqual(call_metadata.value.metadata.query(())['dimension']['length'], 3)
        self.assertEqual(call_metadata.value.metadata.query((base.ALL_ELEMENTS, 0,))['structural_type'], np.int64)
